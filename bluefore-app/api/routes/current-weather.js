...
router.get('/api/current-weather-by-id/:id', (req, res) => {
    // Route params
    ...
    sendCurrentWeatherResponse({id, language, units}, res);
});
...
const sendCurrentWeatherResponse = (args, res) => {
    axios.get(`https://api.openweathermap.org/data/2.5/weather?id=${args.id || ''}&q=${encodeURIComponent(args.location || '')}&lat=${args.latitude || ''}&lon=${args.longitude || ''}&units=${args.units || 'metric'}&lang=${args.language || ''}&appid=${environment.WEATHER_API_KEY}`).then(currentWeather => {
